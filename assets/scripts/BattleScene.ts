...
export class BattleScene extends Component {
  ...
  onLoad() {
    ...
    // 图片前景层（AI 生成的暗色草木剪影）：在角色之前，最前景，随关卡换
    this.makeScrollLayer('bg-fg-forest', 580, 0.9, -512);   // tiles 建在 fgsil 之后=最前
    this.fgLayerIdx = this.layers.length - 1;               // 记住前景层真实索引(石头层在它之前)
